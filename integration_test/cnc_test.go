...
func (i *IntegrationTestSuite) SetupSuite() {
	...
	err := execError.Error
	if err != nil {
		i.Fail("failed to execute docker compose up:", err.Error(), execError.Stdout, execError.Stderr)
	}
}
...
func (i *IntegrationTestSuite) TestDataRoundTrip() {
	...
	txRes, err := client.SubmitPFD(context.TODO(), [8]byte{1, 2, 3, 4, 5, 6, 7, 8}, randomData, 100000)
